...
// 美しいダークモードの地図タイル (CartoDB Dark Matter)
L.tileLayer('https://{s}.basemaps.cartocdn.com/dark_all/{z}/{x}/{y}{r}.png', {
    attribution: '&copy; CARTO',
    subdomains: 'abcd',
    maxZoom: 16,
    minZoom: 4,
    opacity: 0.9 // 地図を少し暗くしてUIを際立たせる
}).addTo(map);

// ズームコントロールを配置（少し内側に）
L.control.zoom({ position: 'topright' }).addTo(map);
const zoomControlContainer = document.querySelector('.leaflet-top.leaflet-right');
zoomControlContainer.style.marginTop = '80px'; // ヘッダーと被らないように
...
const dragonData = [
    ...
    { name: "籠の目", jp: "摩周湖", lat: 43.57, lng: 144.53 },
...
dragonData.forEach((data, index) => {
    // zIndexOffsetを使って、マーカーがビネット効果より手前に来るようにする
    const marker = L.marker([data.lat, data.lng], { icon: dragonIcon, zIndexOffset: 1000 }).addTo(map);
    marker.dragonDataIndex = index;
...
function startTour() {
    ...
    // ボタンのアクティブスタイル
    btnPlay.style.background = 'linear-gradient(135deg, rgba(255, 100, 100, 0.3), rgba(255, 50, 50, 0.5))';
    btnPlay.style.borderColor = '#ff6b6b';
    btnPlay.style.color = '#fff';
    
    if (currentTourIndex === -1 || currentTourIndex >= dragonData.length -1) {
        currentTourIndex = -1;
    }
    playNextStep();
...
function stopTour() {
    isPlaying = false;
    btnPlay.innerHTML = '▶ Start Tour';
    // ボタンのスタイルを元に戻す（CSSに任せるため空文字に）
...
function jumpToStep(index) {
    ...
    map.flyTo([data.lat, data.lng], targetZoom, { duration: 2.5, easeLinearity: 0.2 });
